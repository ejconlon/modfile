...
data Header = Header
  ...
  -- ^ song name (28 bytes)
  , hpad0 :: Word8
  -- ^ should be 0x1a for scream tracker 3 modules
  , magicByte :: Word8
  -- ^ should be 0x10 for scream tracker 3 modules
  , hpad1 :: Word16
  -- ^ padding
  , songLength :: Word16
  -- ^ number of entries in pattern order table
  , numInstruments :: Word16
  -- ^ number of instruments
  , numPatterns :: Word16
  -- ^ number of patterns
  , flags :: Word16
  -- ^ bit 0: scream tracker 2 vibrato, bit 1: scream tracker 2 tempo,
  ...
  -- ^ upper four bits: tracker id, lower 12 bits: version
  ...
  --   bits 6-0: volume
  , clickRemoval :: Word8
  -- ^ number of channels to use for click removal on real GUS hardware
  , defaultPanFlag :: Word8
  -- ^ if 252: read panning values
  , hpad2 :: [Word8]
  -- ^ padding (8 bytes)
  , special :: Word16
  -- ^ unused
  , channelSettings :: [Word8]
  -- ^ channel settings (32 bytes/channels)
  }
  deriving (Show, Eq)
