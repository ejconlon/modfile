...
getCell =
  getWord8
    >>= \n ->
      if not (testBit n 7)
        then
          Cell (Just . toEnum . fromIntegral $ n)
            <$> (Just <$> getWord8)
            <*> (Just <$> getWord8)
            <*> (Just <$> getWord8)
            <*> (Just <$> getWord8)
          ...
          Cell
            <$> getByMask n 0 (toEnum . fromIntegral <$> getWord8)
            <*> getByMask n 1 getWord8
            <*> getByMask n 2 getWord8
            <*> getByMask n 3 getWord8
...
putCell
  ( Cell
      ...
      (Just t)
      (Just p)
    ) = mapM_ putWord8 [toEnum . fromEnum $ n, i, v, t, p]
putCell Cell {..} = do
  putWord8 $
    mask effectParam 4 $
      mask effectType 3 $
 ...
 where
  mask f n m = if isJust f then setBit m n else m
