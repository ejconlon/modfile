...
putModule Module {..} = do
  ...
  replicateM_ (16 - (body `rem` 16)) (putWord8 0)
  mapM_ (uncurry putInstrument) $ zip (scanl (+) (1 + last pPtrs) ((\x -> 1 + x `div` 16) . samSize <$> instruments)) instruments
  mapM_ (\p -> putPattern p >> replicateM_ (16 - ((packedSize p) `rem` 16)) (putWord8 0)) patterns
  mapM_
    ( maybe (return ()) ((\sd -> mapM_ putWord8 $ sd ++ replicate (16 - ((length sd) `rem` 16)) 0) . sampleData) . pcmSample
    )
    instruments
 where
  samSize Instrument {..} = maybe 0 (length . sampleData) pcmSample
